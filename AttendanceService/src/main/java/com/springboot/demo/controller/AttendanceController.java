...
//    }
    ...
    @GetMapping("/history/search/{employeeName}/{userRole}")
    public ResponseEntity<List<Attendance>> getAttendanceHistoryByEmployeeName(
            @PathVariable String employeeName,
            @PathVariable String userRole) {
        return ResponseEntity.ok(attendanceService.getAttendanceHistoryByEmployeeName(employeeName, userRole));
    }
    ...
    @GetMapping("/peak-day")
    public ResponseEntity<Map<String, Object>> getPeakDay(
            @RequestParam String startDate,
            @RequestParam String endDate) {

        Map<String, Object> response = attendanceService.getWeeklyPeakDay(LocalDate.parse(startDate), LocalDate.parse(endDate));
        return ResponseEntity.ok(response);
    }
}
