...
public interface RoomRepo extends JpaRepository<MeetingRoom,Integer> {
    ...
    @Query("SELECT m FROM MeetingRoom m WHERE m.location = :location AND (:floor = 0 OR m.floor = :floor)")
    List<MeetingRoom> findAvailableRooms(@Param("location") String location, @Param("floor") int floor);
